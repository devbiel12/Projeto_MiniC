...
if [ "$#" -ne 2 ]; then
    echo "Uso: bash testar_parser_python.sh <testes-parser-50> <parser.py>" >&2
    exit 2
fi
...
if [ -n "${PYTHON:-}" ]; then
    ...
    if [ "${#PYTHON_CMD[@]}" -eq 0 ]; then
        echo "ERRO: Python não encontrado. Instale Python ou defina PYTHON." >&2
        exit 2
    fi
fi

exec "${PYTHON_CMD[@]}" "$SCRIPT_DIR/test_parser_50.py" "$CASES_DIR"
...
for case_dir in "$CASES_DIR"/*; do
    ...
    "${PYTHON_CMD[@]}" "$PARSER_PATH" "$source" >"$actual" 2>"$error"
    status=$?

    if [ "$id" -le 25 ]; then
        if [ "$status" -eq 0 ] && normalize_ast "$actual" "$expected"; then
            echo "$id | ACEITO"
            passed=$((passed + 1))
        else
            echo "$id | FALHOU"
